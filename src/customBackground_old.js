...
const genColor = () => {
	let hue = Math.floor(getRandom() * 360);
...
const generateColorScheme = (taggedColors, _darkness) => {
	...
	// Generate base color
	{
		...
		saturations.push(70 + getRandom()*20);

		let color = genColorAdv(
			getColorToH(colorToH[colorTag]), 
			saturations[saturations.length-1], 
			55 + Math.min(35,Math.max(0,(15 + darkness*5 - 5 + getRandom()*10)))
			);
	...
	// Generate secondary color
	{
		...
		saturations.push(saturations[saturations.length-1] < 70 ? 70 + getRandom()*10 : 60 + getRandom()*20);

		if(!nonColorUsed && getRandom()<0.1) {
			color = genColorAdv(
				0, 
				0, 
				90 + getRandom()*5
			...
			color = genColorAdv(
				getColorToH(colorToH[colorTag]), 
				saturations[saturations.length-1], 
				65 + Math.min(25,Math.max(0,(15 + darkness*5 - 5 + getRandom()*10)))
				);
		}
	...
	// Generate third color
	{
		...
		if(!nonColorUsed && getRandom()<0.1) {
			...
			color = genColorAdv(
				getColorToH(colorToH[colorTag]), 
				/* brightColors.includes(colorTag) ? 80 + getRandom()*10 : */ 60 + getRandom()*20, 
				//brightColors.includes(colorTag) ? 60 + Math.min(30,Math.max(0,(15 + darkness*5 - 5 + getRandom()*10))) : // bright colors can't be too dark
				60 + Math.min(30,Math.max(0,(15 + darkness*5 - 5 + getRandom()*10)))
				);
		}
...
const generateColorScheme2 = (taggedColors, _darkness) => {
	...
	// Generate base color
	{
		...
		saturations.push(75 + getRandom()*15);
		lightings.push(75 + Math.min(15,Math.max(0,(8 + darkness*3 - 3 + getRandom()*6))));

		/* if(hues[hues.length-1] > colorToH["ORANGE"][0] && hues[hues.length-1] < colorToH["ORANGE"][1]) {
			saturations[saturations.length-1] = 80 + getRandom()*10;
			lightings[lightings.length-1] = 80 + getRandom()*10;
		} */
	...
	// Generate secondary color
	{
		if(!nonColorUsed && getRandom()<0.2) {
			...
			let colorH = hues[hues.length-1] + (getRandom() > 0.5 ? +(30 + getRandom()*30): -(30 + getRandom()*30));
			colorH = colorH > 360 ? colorH - 360 : colorH<0 ? colorH+360:colorH;
			hues.push(colorH);
			saturations.push(80 + getRandom()*10);
			lightings.push(60 + Math.min(15,Math.max(0,(8 + darkness*3 - 3 + getRandom()*6))));

			/* if(saturations[saturations.length-2] >= 80 && saturations[saturations.length-1] >= 80) {
				saturations[saturations.length-1] = 70 + getRandom()*8;
			} */
			
			/* if(lightings[lightings.length-2] > 80) {
				lightings[lightings.length-1] = 70 + (Math.min(25,Math.max(0,(15 + darkness*5 - 5 + getRandom()*10))))/2;
			} */
	...
	// Generate third color
	{
		...
		if(!nonColorUsed && getRandom()<0.1) {
			...
			hues.push(getColorToH(colorToH[colorTag]));
			saturations.push(70 + getRandom()*20);
			lightings.push(70 + (Math.min(25,Math.max(0,(15 + darkness*5 - 5 + getRandom()*10))))/2);
